...
model_pool = ["MLP", "resnet18", "bert"]
optimizer_pool = ["sgd", "adam"]
metric_pool = ["std", "EER", "EO", "DP", "no_metrics"]
method_pool = ['vanilla', 'FSW', "FSS", 'joint', 'finetune', 'AGEM', "GSS", "iCaRL", "WA", "OCS", "FaIRL", "CLAD"]
...
def parse_option():
    ...
    parser.add_argument('--token_max_length', type=int, default=32,
                        help='set natual language input token length.')
    ...
    parser.add_argument('--round', type=bool, default=False, 
                        help='True then round the weight and make the weight binary')
    ...
    # other fairness loss
    parser.add_argument('--fairloss', type=str, default="CE", choices=["CE", "hinge"],)
...
def make_params(args) -> dict:
    ...
    if params['dataset'] == "Bios":
        trial_id += f"_{params['token_max_length']}"

    # per_task_memory_examples_32: proper per_task_memory_examples if we store 32 samples per sensitive group
    if params['dataset'] == "BiasedMNIST":
        per_task_memory_examples_32 = 128
    elif params['dataset'] == "MNIST":
        per_task_memory_examples_32 = 64
    elif params['dataset'] == "FashionMNIST":
        per_task_memory_examples_32 = 64
    elif params['dataset'] == "Drug":
        per_task_memory_examples_32 = 64
    elif params['dataset'] == "Bios":
        per_task_memory_examples_32 = 320        
    ...
    if params['method'] == 'CLAD':
        trial_id+=f"_{params['eta']}"

    if params['optimizer'] != "sgd":
        trial_id+=f"_{params['optimizer']}"
    ...
    if not per_task_memory_examples_32 == params["per_task_memory_examples"]:
        ratio = params["per_task_memory_examples"]/per_task_memory_examples_32
        trial_id+=f"_buf=x{ratio}"

    if not "CE" == params["fairloss"]:
        trial_id+=f"_fairloss={params['fairloss']}"
