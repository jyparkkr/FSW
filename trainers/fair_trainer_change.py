...
class FairContinualTrainer1(ContinualTrainer1):
    ...
    def validate_algorithm_on_task(self, task: int, validate_on_train: bool = False) -> Dict[str, float]:
        ...
        # to compare btw 0-1 loss and CE loss
        class_loss = dict()
        class_loss_s0 = dict()
        class_loss_s1 = dict()
        ...
        with torch.no_grad():
            for items in eval_loader:
                ...
                loss = criterion(prob, targ.reshape(-1)).cpu()
        ...
        if sen_dataset:
            ...
            CEloss_eo = [abs(avg_(class_loss_s0[c]) - avg_(class_loss_s1[c])) for c in class_loss.keys()]
        ...
        return {'accuracy': avg, 'loss': test_loss, "std": std, "EER": -1, 
                'EO': multiclass_eo, 'CE_EO': CEloss_eo, 'DP': -1, 
