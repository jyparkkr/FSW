...
# This file was *autogenerated* from the file solid_angle.sage
...
import os
load(os.path.join("/mnt/shared/jaeyoung/cl_gym", "solid_angle_code/logging.sage"))
...
def solid_angle_simplicial_2d(A):
    ...
    if not hasattr(A, 'nrows'):
        A = matrix(A)
    if A.nrows() != _sage_const_2  or A.ncols() != _sage_const_2 :
        raise ValueError("input matrix has incorrect dimension.")
    u = A.row(_sage_const_0 )
    v = A.row(_sage_const_1 )
    p = u.dot_product(v)
    a = u.norm()
    b = v.norm()
    cs = p/(a*b)
    final_calc = arccos(cs) / (_sage_const_2 *pi)
...
def solid_angle_simplicial_arccos_3d(A):
    ...
    if not hasattr(A, 'nrows'):
        A = matrix(A)
    if A.nrows() != _sage_const_3  or A.ncols() != _sage_const_3 :
        raise ValueError("input matrix has incorrect dimension.")
    v_0 = A.row(_sage_const_0 )
    v_1 = A.row(_sage_const_1 )
    v_2 = A.row(_sage_const_2 )
    ...
    return RDF((a_0+a_1+a_2 - pi)/(_sage_const_4 *pi))
...
def solid_angle_simplicial_arctan_3d(A):
    ...
    if A.nrows() != _sage_const_3  or A.ncols() != _sage_const_3 :
        raise ValueError("input matrix has incorrect dimension.")
    vnorm = [A[i].norm() for i in range(_sage_const_3 )]
    a = A[_sage_const_0 ]/vnorm[_sage_const_0 ]
    b = A[_sage_const_1 ]/vnorm[_sage_const_1 ]
    c = A[_sage_const_2 ]/vnorm[_sage_const_2 ]
    w = matrix([a, b, c])
    det = abs(w.determinant())  # same as det of matrix [abc]
    dab = a.dot_product(b)
    dac = a.dot_product(c)
    dbc = b.dot_product(c)
    denom = _sage_const_1 +dab+dac+dbc
    omega = _sage_const_2 *atan2(det, denom)
    return RDF(omega/(_sage_const_4 *pi))
...
# **********************************************************
def solid_angle_general(A, eps=_sage_const_1en6 , deg=_sage_const_100 ,
    ...
    if simplicial is True:
        ...
        da = int(d * (d-_sage_const_1 ) / _sage_const_2 )
        const = sqrt((v * v.transpose()).determinant()) / (RDF(_sage_const_4 *pi) ** (d/_sage_const_2 ))
        alpha = [_sage_const_0 ] * da
        for i in range(d - _sage_const_1 ):
            for j in range(i + _sage_const_1 , d):
                k = (_sage_const_2 *d - i - _sage_const_1 ) * i/_sage_const_2  + j - i - _sage_const_1 
                alpha[k] = v[i] * v[j]
        partial_sum = _sage_const_0 
        for n in range(deg + _sage_const_1 ):
            sum_deg_n = _sage_const_0 
            for a in composition_of_n_into_k_parts(n, da):
                alphatoa = _sage_const_1 
                for k in range(da):
                    alphatoa = alpha[k] ** a[k] * alphatoa
                    if alphatoa == _sage_const_0 :
                        break
                if alphatoa == _sage_const_0 :
                    continue
                t = (-_sage_const_2 ) ** (sum(a))
                ...
                for i in range(d):
                    ...
                    for j in range(d):
                        if j != i:
                            m_1 = max(i, j)
                            m_0 = min(i, j)
                            k = (_sage_const_2 *d - m_0 - _sage_const_1 ) * m_0 / _sage_const_2  + m_1 - m_0 - _sage_const_1 
                            s_i += a[k]
                    coef = coef * gamma(_sage_const_0p5  * (s_i + _sage_const_1 ))
...
def simplicial_subcones_decomposition(A):
    ...
    if A.rank() == A.nrows():
        ...
        origin = A.nrows()
        pc = PointConfiguration(A.stack(vector([_sage_const_0 ]*A.ncols())), star=origin)
...
def composition_of_n_into_k_parts(n, k):
    ...
    if k == _sage_const_1 :
        ...
        yield [_sage_const_0 ] * k
    else:
        for i in range(n+_sage_const_1 ):
            for c in composition_of_n_into_k_parts(n-i, k-_sage_const_1 ):
                yield [i]+c
